...
class VideoConcatenator:
    def __init__(self, result_directory_path, first_video_path, second_video_path,
        ...
        self.folder_path = "data/temp_videos"
    ...
    def create_video(self):
        ...
        for index, video_input in enumerate(concat_video_list, start=1):
            scaled_stream = video_input.filter('scale', f'{self.__video_width}x{self.__video_height}').filter('setsar', self.__sar)
            output_file = f"{self.folder_path}/temp_vid_{index}.mp4"
            ffmpeg.output(scaled_stream, output_file, b=f"{self.bitrate}k").run()
            scaled_videos.append(ffmpeg.input(output_file))
            self.update_progress(index)

        output_file = self.result_directory_path + f'/{self.result_name}.mp4'
        ffmpeg.concat(*scaled_videos).output(output_file).run()
